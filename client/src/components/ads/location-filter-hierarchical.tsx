...
interface LocationFilterHierarchicalProps {
  value: string | null;
...
export default function LocationFilterHierarchical({ value, onChange }: LocationFilterHierarchicalProps) {
    ...
    <div className="bg-white dark:bg-gray-900 rounded-md border border-gray-200 dark:border-gray-700 overflow-hidden">
      {showCities && selectedVoivodeship ? (
        <div>
          ...
          <Button 
            variant="ghost" 
            className="flex items-center w-full justify-start p-3 rounded-none border-b border-gray-200 dark:border-gray-700" 
            onClick={handleBack}
          >
          ...
          {/* Lista miast */}
          <div>
            <h4 className="px-3 pt-3 text-sm font-semibold text-gray-500 dark:text-gray-400">
              Wybierz miasto
            </h4>
            <div className="max-h-72 overflow-y-auto">
              {CITIES_BY_VOIVODESHIP[selectedVoivodeship].map((city) => (
                <button
                  key={city}
                  className={`w-full text-left p-3 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 ${
